...
export function LandingFeatures() {
  ...
  const locale = useLocale() as (typeof appConfig.locales)[number]
  return (
    <LandingSection>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 lg:gap-8">
        {features.map(({ locales, icon: Icon }, index) => (
          <Card
            key={index}
          ...
          >
            <CardHeader>
              ...
              />
              <div className="space-y-2 text-center">
                <h3 className="text-xl font-bold">{locales[locale].title}</h3>
                <p className="text-zinc-500 dark:text-zinc-400">
                  {locales[locale].details}
                </p>
              </div>
            </CardHeader>
          </Card>
...
// Use all locales to make a type
interface Locales {
